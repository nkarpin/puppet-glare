...
class glare::backend::swift(
  ...
  glare_config {
    'glare_store/swift_store_region':         value => $swift_store_region;
    'glare_store/swift_store_container':      value => $swift_store_container;
    'glare_store/swift_store_create_container_on_put':
      value => $swift_store_create_container_on_put;
    'glare_store/swift_store_large_object_size':
      value => $swift_store_large_object_size;
    'glare_store/swift_store_endpoint_type':
      value => $swift_store_endpoint_type;

    'glare_store/swift_store_config_file':    value => '/etc/glare/glare-swift.conf';
    'glare_store/default_swift_reference':    value => $default_swift_reference;
  }

  if !$multi_store {
    glare_config { 'glare_store/default_store': value => 'swift'; }
  }
