...
class glare::backend::cinder(
  ...
  glare_config {
    'glare_store/cinder_api_insecure':           value => $cinder_api_insecure;
    'glare_store/cinder_catalog_info':           value => $cinder_catalog_info;
    'glare_store/cinder_http_retries':           value => $cinder_http_retries;
    'glare_store/cinder_endpoint_template':      value => $cinder_endpoint_template;
    'glare_store/cinder_ca_certificates_file':   value => $cinder_ca_certificates_file;
  }

  if !$multi_store {
    glare_config { 'glare_store/default_store': value => 'cinder'; }
  }
